...
var DateUtil = {
	string2date: function(str) {//字符串转日期
		if(str.search(/^\d{4}\D\d{1,2}\D\d{1,2}$/)==-1){
			throw new Error("日期格式错误，请检查");
		}
		//str.replace(/\D/g,"-");  //简单写法
		str = str.replace(/(\d+)\D/g,function(matched, sub1, index){
			return sub1+"-";
		});
		return new Date(str);
	},
	...
	getDayAfter: function(n) { //获得N天以后的日期
		if(n==undefined || isNaN(Number(n)) ) {
			throw new Error("你的参数是不是没传？  或者写错内容了！  请仔细检查");
		}
		var now = new Date();
...
function offsetPage(obj) {
	...
	if( !(obj instanceof Element) ) {
		throw new Error("你的元素类型错误，必须是DOM元素!");
	}
